...
const timeOutFreeSpot = 7000; // in milisecond
...
const makeBill = (car) => {
  ...
  let billValue = car.isParked ? "-" : "Rp " + (hoursBilled <= 1) ? payFirstHour : (payFirstHour + (hoursBilled * payPerHour));
  return billValue;
}
...
const freeSpot = (event) => {
  ...
  if (cars.length == totalPlaces) {
    ...
    document.querySelector('#carButton').removeAttribute('disable');
  }
